...
When you hear the acronym _RTC_, some form of video conferencing probably comes to mind. But buried in Agora's RTC SDK is a feature that changes how you can build real-time multiplayer games: **data messaging**. Instead of setting up WebSockets, managing reconnections, or dealing with a separate backend for game state, you can piggyback game logic on the same client and infrastructure handling your audio and video.
...
- The game can still be played via mouse clicks
